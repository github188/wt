...
int get_config()
{
	...
	if( get_ini(buf, "platform_ac_port", temp) ){
		xyprintf(0, "Get config from ini file is failed -- %s %d", __FILE__, __LINE__);
		return -1;
	}
	...
	xyprintf(0, "proxy_port = %d\n\tproxy_addr = %s\n\tplatform_port = %d",
			cgv_proxy_port, cgv_proxy_addr, cgv_platform_port);
	//获取数据库内的配置内容
	...
	//3.2绑定变量和SQL语句查询结果
	SQLBindCol(handle->sqlstr_handle, 1,	SQL_C_ULONG,	&sgv_ac_port,		20,		&handle->sql_err);
	SQLBindCol(handle->sqlstr_handle, 2,	SQL_C_ULONG,	&sgv_ac_interval,	20,		&handle->sql_err);

	//3.3执行SQL语句
	sprintf(handle->sql_str, "SELECT TOP 1 acserverport, acacservertime FROM mx_rconfig");
	...
	xyprintf(0, "ac_interval = %d", sgv_ac_interval);
	xyprintf(0, "ac_port = %u", sgv_ac_port);

	wt_sql_destroy(handle);
	free(handle);

	if(sgv_ac_port <= 0){
		xyprintf(0, "WARNING:Can't find the ac_port in mx_rconfig, used 58862!");
		sgv_ac_port = 58862;
	}
