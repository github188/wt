...
#if ROUTER_DEBUG
		xyprintf(0, "%u:--> --> Return the state message success --> -->, count = %u", msg_head->deviceID, heart_count);
#endif
...
void* epoll_thread(void *fd)
{
	...
	while(1){
		...
		for( pos = gv_sock_list_head.next; pos != &gv_sock_list_head; pos = pos->next ){//遍历设备list 添加或删除设备到epoll队列中
			...
			if( time(0) - sock->last_heart_time > sgv_ac_interval + 60 ){	//设备的最后活动时间 超过两次间隔时间则在下面删除
...
void* ac_process_fun(void *fd, wt_sql_handle *handle)
{
	...
	//if( msg->randID == 0 && msg->needPWD == 77 ){
		//3.2绑定变量和SQL语句查询结果
		__u32 id = 0, shanghuid = 0;
		...
		//3.3执行SQL语句
		sprintf(handle->sql_str, "SELECT TOP 1 id, shanghuid from mx_shebei WHERE hard_seq = '%s'", msg->sn);
...
void* accept_thread(void* fd)
{
	...
	int sockfd;
	if( wt_sock_init( &sockfd, sgv_ac_port, MAX_EPOLL_NUM ) ){
		goto ERR;
	}
